    ...
    for root, dirs, files in os.walk(file_dir):
        ...
        for file in files:
            if '.h' in file:#file_dir+'/'+file含义是file_dir文件夹下的file文件
                HFileAddMj(file_dir+'/'+file, "h;","o;","w;", "d;","n;","e;","r;")#往凡是以"l;", "m;","n;","q;","y;"这些中的某一个结尾的oc语句后添加费代码
            if '.m' in file:
                MFileAddMj(file_dir+'/'+file, "e];", "w];","m];","c];","s];","o];","k];")#往凡是以"n];", "w];","m];","c];","p];","q];","l];"这些中的某一个结尾的oc语句后添加费代码
#要修改的文件所在的文件夹路径
